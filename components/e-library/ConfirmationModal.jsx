...
const ConfirmationModal = ({
  ...
  const current = colors[variant] || colors.danger;
    ...
    >
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-md max-h-full overflow-y-auto transform transition-all duration-300 scale-95 opacity-0 animate-fade-in-scale">
        <div className="p-6 sm:flex sm:items-start">
          <div
            className={`mx-auto flex-shrink-0 flex items-center justify-center h-12 w-12 rounded-full ${current.iconContainer} sm:mx-0 sm:h-10 sm:w-10`}
          >
            <svg
        ...
        <div className="p-4 bg-slate-50 rounded-b-xl flex justify-end items-center gap-4">
          ...
          <button
            type="button"
            onClick={onConfirm}
            className={`px-4 py-2 border border-transparent rounded-md text-sm font-semibold text-white ${current.button} focus:outline-none focus:ring-2 focus:ring-offset-2`}
          >
            {confirmText}
          </button>
        </div>
