...
const ReportsView: React.FC = () => {
  ...
  const totalEnrolled = MOCK_STUDENT_DATA.filter(
    (s) => s.status === "Enrolled"
  ).length;
  ...
  const overallAttendance = useMemo(() => {
    const present = MOCK_ATTENDANCE_DATA.filter(
      (r) =>
        r.status === AttendanceStatus.PRESENT ||
        r.status === AttendanceStatus.LATE
    ).length;
    return MOCK_ATTENDANCE_DATA.length > 0
      ? `${((present / MOCK_ATTENDANCE_DATA.length) * 100).toFixed(1)}%`
      : "N/A";
  }, []);
  ...
  const generateSummary = async () => {
    ...
    try {
      ...
      setError(
        err.message ||
          "An unexpected error occurred. Please check your API key and network connection."
      );
    } finally {
      setIsLoading(false);
    }
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-2">
        <div>
          ...
          <p className="text-slate-500">
            An overview of school-wide data trends and key performance
            indicators.
          </p>
        </div>
        <button
